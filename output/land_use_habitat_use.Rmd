...
(sf_recoverydomain_use <- st_read(here("./data-nogit/ESU_hucdata/huc6_esu-withadds-ver5-alb27.shp")) %>% clean_names()) %>% names
...
ex <- do.call(rbind,ex)
result_r_ch <- ex  %>% rename(name = r_ch) %>% group_by(name, hab_use, spawning, rearing, migration) %>% summarise(
...
save(land_use_habitat_use, file = here("./output/land_use_habitat_use.RData")) #save data for use without running extract code, which is slow
write.dta(land_use_habitat_use, here("./output/land_use_habitat_use.dta")) #save data as Stata file for Lisa
