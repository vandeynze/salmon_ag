...
# wd <- "C:/Users/Lisa.Pfeiffer/Documents/GitHub/salmon_ag/" #Braeden--it's telling me not to use a setwd in chunks but I'm not sure what to do instead, could you advise?
 # setwd(wd)
# When we open the Rstudio project before opening the .R/.Rmd (check upper right cube symbol in Rstudio) the wd is automatically set to where the .Rproj file lives
# To access files further down, use "./" relative directories, or the `here` notation e.g. `here("output/file.csv")`

# Load habitat use recovery domain shape file
(sf_recoverydomain_use <- st_read(here("/data/ESU_hucdata/huc6_esu-withadds-ver5-alb27.shp")) %>% clean_names()) %>% names
...
raster_cdl_merge <- raster(here("/data/cdl_west_prof.tif"))
...
if(!file.exists("./output/land_use_habitat_use.RData")) {
...
save(land_use_habitat_use, file = "./output/land_use_habitat_use.RData") #save data for use without running extract code, which is slow
write.dta(land_use_habitat_use, "./output/land_use_habitat_use.dta") #save data as Stata file for Lisa
...
```{r figure, echo=FALSE}
# setwd(wd)
load(here("/output/land_use_habitat_use.RData")) #load data if not already loaded

#Feel free to format better!
...
Notice that in the Upper Willamette River Chinook Salmon figure, a large amount of crop coverage is "other crops". This table lists the frequency of the crops in this category. This confirms that it is the Sod/Grass seed category as Braeden suspected. 
#Please make this table better...I couldn't figure out how to sort it! :/
